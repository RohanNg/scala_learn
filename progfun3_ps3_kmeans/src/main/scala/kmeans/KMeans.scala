...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    // never modify GenThing, only read for Threadsafe
    // never modify those collection during data parallel operation
    // never read from currently modified collection

    // Thread safety argument
    //    means is never modified
    //
    var map = points.groupBy(findClosest(_, means))
    for (mean <- means.seq) {
      // ensure that the map contain all key
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.seq.map(old => findAverage(old, classified(old)))
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    assert(oldMeans.length == newMeans.length)
    val oldMeansIt = oldMeans.iterator
    val newMeansIt = newMeans.iterator
    var i = 0
    while(i < oldMeans.length) {
      if(oldMeansIt.next().squareDistance(newMeansIt.next()) >= eta)
        return false
      i = i+ 1
    }
    true
